...
class AnnouncementAdmin(admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form.base_fields["addedBy"].queryset = User.objects.filter(
            groups__name__in=("Kadra", "Sztab", "Bajer")
        )
        return form
...
class BusAdmin(ImportExportModelAdmin, admin.ModelAdmin):
    ...
    def user_count_to(self, obj):
        return obj.user_set.filter(
            bus_info__in=(User.BusInfoChoices.BOTH, User.BusInfoChoices.TO)
        ).count()
    ...
    def user_count_return(self, obj):
        return obj.user_set.filter(
            bus_info__in=(User.BusInfoChoices.BOTH, User.BusInfoChoices.RETURN)
        ).count()
