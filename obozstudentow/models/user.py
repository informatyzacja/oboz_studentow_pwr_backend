...
class User(AbstractUser):
    ...
    photo = ResizedImageField(
        upload_to="users", blank=True, null=True, force_format=None
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.house
            and self.house.chat
            and not self.house.chat.users.filter(pk=self.pk).exists()
        ):
...
class TinderAction(models.Model):
    ...
    target = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="tinderaction_target"
    )
