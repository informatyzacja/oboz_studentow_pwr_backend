...
class Group(Orderable):
    ...
    background = models.ImageField(
        upload_to="groups/backgrounds", blank=True, null=True
    )
...
class GroupMember(models.Model):
    ...
    def __save__(self, *args, **kwargs):
        ...
        if (
            self.group.chat
            and not self.group.chat.users.filter(pk=self.user.pk).exists()
        ):
...
class Point(models.Model):
    ...
    addedBy = models.ForeignKey(
        "obozstudentow.User", on_delete=models.SET_NULL, null=True
    )
