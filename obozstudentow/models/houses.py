...
class House(models.Model):
    ...
    user_key_collected = models.ForeignKey(
        "User",
    ...
    floor = models.CharField(
        max_length=50, default=None, verbose_name="Piętro", blank=True, null=True
    )
    description = models.TextField(
        default=None, verbose_name="Opis", blank=True, null=True
    )
    signup_open = models.BooleanField(
        default=True, verbose_name="Czy można się zapisać?"
    )
    signout_open = models.BooleanField(
        default=True, verbose_name="Czy można się wypisać?"
    )

    chat = models.ForeignKey(
        Chat, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Czat"
    )
...
class HouseSignupProgress(models.Model):
    ...
    def seconds_until_free(self):
        ...
        return max(
            0,
            (
                (timezone.timedelta(seconds=60) + self.last_updated) - timezone.now()
            ).total_seconds(),
        )
