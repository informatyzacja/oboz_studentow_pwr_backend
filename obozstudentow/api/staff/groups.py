...
def get_group(request):
    ...
    try:
        group = Group.objects.get(id=group_id)
        return Response(
            GroupWithMembersSerializer(group, context={"request": request}).data
        )
    except Group.DoesNotExist:
...
def get_groups(request):
    return Response(
        GroupSerializer(
            Group.objects.filter(~Q(type__name="Frakcja")),
...
def get_fractions(request):
    return Response(
        GroupSerializer(
            Group.objects.filter(type__name="Frakcja").order_by("sort_order"),
            context={"request": request},
            many=True,
        ).data
    )
