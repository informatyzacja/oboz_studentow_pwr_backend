...
def validate_points(request, id):
    if not Point.objects.filter(id=id).exists():
        return Response(
            {"success": False, "error": "Nie znaleziono punktów o podanym id"}
        )
...
def add_points(request):
    if (
        ...
        or not request.data["numberOfPoints"]
    ):
        return Response(
            {"success": False, "error": "Nie podano wszystkich wymaganych danych"}
        )
    if not Group.objects.filter(id=request.data["group"]).exists():
        return Response(
            {"success": False, "error": "Nie znaleziono grupy o podanym id"}
        )
    if not PointType.objects.filter(id=request.data["type"]).exists():
        return Response(
            {"success": False, "error": "Nie znaleziono typu punktów o podanym id"}
        )
    point = Point()
