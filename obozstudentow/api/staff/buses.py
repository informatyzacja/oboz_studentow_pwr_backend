...
def get_buses(request):
    ...
    if bus_presence_type not in ("to", "return"):
        return Response(
            {
                "success": False,
                "error": "Sprawdzanie obecności w busach nie jest aktywowane",
            }
        )
    bus_presence_to = bus_presence_type == "to"

    users_own_transport = User.objects.filter(
        Q(bus__isnull=True)
        | (
            Q(bus_info=User.BusInfoChoices.RETURN)
            if bus_presence_to
            else Q(bus_info=User.BusInfoChoices.RETURN)
        )
    )
    users_count = users_own_transport.count()
    present_users_count = users_own_transport.filter(
        Q(bus_presence=True) if bus_presence_to else Q(bus_presence_return=True)
    ).count()
    opaski_count = users_own_transport.filter(
        bandId__isnull=False, bandId__gte=300000
    ).count()
        ...
        list(
            ...
            .annotate(
                users_count=Count(
                    ...
                    filter=Q(
                        user__bus_info__in=(
                            User.BusInfoChoices.BOTH,
                            User.BusInfoChoices.TO,
                        )
                    ),
                )
                if bus_presence_to
                    ...
                    filter=Q(
                        user__bus_info__in=(
                            User.BusInfoChoices.BOTH,
                            User.BusInfoChoices.RETURN,
                        )
                    ),
                ),
                present_users_count=Count(
                    "user",
                    filter=Q(
                        user__bus_presence=True,
                        user__bus_info__in=[
                            User.BusInfoChoices.BOTH,
                            User.BusInfoChoices.TO,
                        ],
                ...
                if bus_presence_to
                    ...
                    "user",
                    filter=Q(
                        user__bus_presence_return=True,
                        user__bus_info__in=[
                            User.BusInfoChoices.BOTH,
                            User.BusInfoChoices.RETURN,
                        ],
                ...
                opaski_count=Count(
                    "user",
                    filter=Q(
                        user__bandId__isnull=False,
                        user__bus_info__in=[
                            User.BusInfoChoices.BOTH,
                            User.BusInfoChoices.TO
                            if bus_presence_to
                            else User.BusInfoChoices.RETURN,
                        ],
                    ),
                ),
            )
...
def get_bus_users(request):
    ...
    if (
        request.GET["bus_id"] != "0"
        and not Bus.objects.filter(id=request.GET["bus_id"]).exists()
    ):
    ...
    if Bus.objects.filter(id=request.GET["bus_id"]).exists():
        ...
        return Response(
            ...
            .order_by(presence_type, "bandId_isnull", "last_name", "first_name")
            .values(
                "id",
                "first_name",
                "last_name",
    ...
    return Response(
        {
            ...
            "presence": user.bus_presence
            if bus_presence_type == "to"
            else user.bus_presence_return,
        }
    )
...
def set_user_band_id(request):
    ...
    if request.user.has_perm("obozstudentow.can_change_bands") == False:
        if user.bandId is not None and int(user.bandId) >= 300000:
            return Response(
                {"success": False, "error": "Użytkownik ma już przypisaną opaskę"}
            )
    ...
    if User.objects.filter(bandId=request.data["band_id"]).exists():
        return Response(
            {
                "success": False,
                "error": "Opaska jest już przypisana do innego użytkownika: "
                + User.objects.get(bandId=request.data["band_id"]).first_name
                + " "
                + User.objects.get(bandId=request.data["band_id"]).last_name,
            }
        )
