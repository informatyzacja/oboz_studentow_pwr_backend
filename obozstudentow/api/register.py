...
def send_verification_email(user: User):
    ...
    html_message = render_to_string(
        "confirmation_email.html", {"code": user.verification_code}
    )
...
def send_email_verification(request):
    ...
    if not email:
        return Response({"error": "No email or first name"})
    ...
    if not user:
        return Response(
            {
                "exists": False,
                "error": "Podany adres e-mail nie jest przypisany do żadnego uczestnika obozu.",
            }
        )
    ...
    if is_constant_code_user(user):
        user.verification_code_valid_until_datetime = (
    ...
    if (
        ...
        or user.verification_code_valid_until_datetime < timezone.now()
    ):
...
def login_with_code(request):
    ...
    user = User.objects.filter(
        ...
        verification_code_valid_until_datetime__gte=timezone.now(),
    ).first()
