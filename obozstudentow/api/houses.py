    ...
    if active and Setting.objects.get(name="house_signup_start_datetime").value:
        active = (
            timezone.datetime.strptime(
                Setting.objects.get(name="house_signup_start_datetime").value,
                "%Y-%m-%d %H:%M",
            )
            <= timezone.now()
        )
...
def signup_user_for_house(request, id):
    room_instead_of_house = (
        Setting.objects.get_or_create(
            name="room_instead_of_house",
            defaults={
                "value": "false",
                "description": 'Czy zamienić nazwę "domek" na "pokój" w opisach?',
            },
        )[0].value.lower()
        == "true"
    )
    ...
    if not house_signups_active():
        return Response(
            {
                "success": False,
                "error": f'Zapisy na {"pokoje" if room_instead_of_house else "domki"} są obecnie wyłączone',
            }
        )
    ...
    if not House.objects.filter(id=id).exists():
        return Response(
            {
                "success": False,
                "error": f'Nie znaleziono {"pokoju" if room_instead_of_house else "domku"} o podanym id',
            }
        )
    ...
    with transaction.atomic():
        ...
        if not house.signup_open:
            return Response(
                {
                    "success": False,
                    "error": f'Zapisy do tego {"pokoju" if room_instead_of_house else "domku"} są obecnie zamknięte',
                }
            )
        ...
        if house.full():
            return Response(
                {
                    "success": False,
                    "error": f'{"Pokój" if room_instead_of_house else "Domek"} jest już pełny',
                }
            )
        ...
        if user != request.user and user.house is not None:
            if user.house == house:
                return Response(
                    {
                        "success": False,
                        "error": f'Ten użytkownik jest już zapisany do tego {"pokoju" if room_instead_of_house else "domku"}',
                    }
                )
        ...
        if user.house == house:
            return Response(
                {
                    "success": False,
                    "error": f'Jesteś już zapisany/a do tego {"pokoju" if room_instead_of_house else "domku"}',
                }
            )
            ...
            if progress.house != house:
                async_to_sync(channel_layer.group_send)(
                    "house-signups",
                    {
                        ...
                        "locators_data": HouseLocatorSerializer(
                            User.objects.filter(house=progress.house), many=True
                        ).data,
                    },
                )
                progress.delete()

        progress, created = HouseSignupProgress.objects.get_or_create(
            house=house, defaults={"user": request.user}
        )
        ...
        if progress.user != request.user and not progress.free():
            return Response(
                {
                    "success": False,
                    "error": f'Ktoś inny aktualnie zapisuje się do tego {"pokoju" if room_instead_of_house else "domku"}',
                }
            )
        ...
        async_to_sync(channel_layer.group_send)(
            "house-signups",
            {
                ...
                "locators_data": HouseLocatorSerializer(
                    User.objects.filter(house=house), many=True
                ).data,
            },
        )
...
def leave_house(request):
    ...
    if request.user.house is None:
        return Response(
            {
                "success": False,
                "error": f'Nie jesteś zapisany do żadnego {"pokoju" if room_instead_of_house else "domku"}',
            }
        )
    ...
    if not house.signout_open:
        return Response(
            {
                "success": False,
                "error": f'Wypisywanie się z tego {"pokoju" if room_instead_of_house else "domku"} jest zablokowane',
            }
        )
...
def get_house_signups_info(request):
    ...
    return Response(
        {
            "room_instead_of_house": room_instead_of_house,
            "house_signups_active": house_signups_active(),
            "house_signup_start_datetime": Setting.objects.get(
