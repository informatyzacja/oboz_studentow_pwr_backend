...
def signup_group(request):
    with transaction.atomic():
        ...
        night_game_start = timezone.datetime.strptime(Setting.objects.get(name='night_game_date').value, '%Y-%m-%d')
        ...
        if request.user.birthDate and (night_game_start.year - request.user.birthDate.year - ((night_game_start.month, night_game_start.day) < (request.user.birthDate.month, request.user.birthDate.day)) < 18):
        ...
        elif GroupMember.objects.filter(user=request.user, group__type=group_type).exists():
            nightGameSignup.failed = True
            nightGameSignup.error = "Użytkownik jest już w jakiejś grupie"
            nightGameSignup.save()
            group.delete()
            return Response({'success':False, 'error': f"Użytkownik {request.user.first_name} jest już w jakiejś grupie",'error_code': 8})
        else:
        ...
        #send notifications
        try:
            tokens = list(UserFCMToken.objects.filter(user__notifications=True, user__in=GroupMember.objects.filter(group=group).values_list('user', flat=True)).values_list('token', flat=True))
