...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            bereal_active = Setting.objects.get(name="bereal_active").value.lower() == "true"
        ...
        if BeerealNotification.objects.filter(date=today).exists() and not options['force']:
            self.stdout.write(self.style.WARNING('BeReal notification already sent today'))
            return
        ...
        notification = BeerealNotification.objects.create(
            date=today,
            deadline=deadline
        )

        # Get all user FCM tokens
        tokens = list(UserFCMToken.objects.filter(
